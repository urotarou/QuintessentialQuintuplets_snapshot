...
import lab.uro.kitori.quintessentialquintuplets.presentation.screen.searchresult.SearchResultAdapter
...
data class TweetViewHolder(
    ...
    companion object {
        fun create(parent: ViewGroup) = TweetViewHolder(DataBindingUtil.inflate(
                LayoutInflater.from(parent.context), RecyclerViewType.TWEET.viewType, parent, false))
    }

    fun bind(item: Tweet?, heroine: Heroine, tweetActionListener: SearchResultAdapter.TweetActionListener) {
        item ?: return

        val tweetUrl = binding.root.context.getString(R.string.link_to_tweet, item.user?.screenName, item.id)
        ...
        binding.nameTextView.text = binding.root.context.getString(R.string.tweet_user, item.user?.userName, item.user?.screenName)
        ...
        if (mediaUrl != null) {
            ...
            binding.illustrationImageView.setBackgroundColor(getHeroineColor(binding.root.context, heroine))
    ...
    private fun getHeroineColor(context: Context, heroine: Heroine) = ContextCompat.getColor(context, heroine.colorResId)
}
