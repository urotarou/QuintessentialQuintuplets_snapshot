...
fun String.toTweetDateTime(context: Context): String {
    ...
    if (betweenDays > 0) return tweetDateTime.format(DateTimeFormatter.ofPattern("yyyy/MM/dd", Locale.getDefault()))
    ...
    if (betweenMinutes > 0) return context.getString(R.string.twitter_date_time_minutes, betweenMinutes)

    val betweenSeconds = ChronoUnit.SECONDS.between(tweetDateTime, currentDateTime)
    if (betweenSeconds >= 10) return context.getString(R.string.twitter_date_time_seconds, betweenSeconds)
